...
// PERUBAHAN BARU //
    ...
    // if ($data['txtbatch_no'] == 'PD-3969A-1') {
...
//    $counter= round($dtbatch['complete_batch'] / $item) + 1;
    $items=$this->transaction_m2->get_item2($data['txtbatch_no'],$counter);
    $item_proses = 0;
    $item = $items->num_rows();
...
<!-- begin breadcrumb -->
  <ol class="breadcrumb pull-right">
    ...
    <li class="breadcrumb-item active">Transaction</li>
      ...
      <!-- end breadcrumb -->
      <!-- begin page-header -->
<h3 class="page-header"><b>Transaction Of WIP</b> || <?php echo $data['txtItemDesc']; ?></h3>
<!-- begin card -->
            <!-- begin row -->
      <div class="row">
        <!-- begin col-3 -->
        <div class="col-lg-3 col-md-6">
          <div class="widget widget-stats bg-white text-inverse">
            <div class="stats-content">
              ...
              <div class="col-md-9">
              <input type="text" name="batch_no" value="<?=$data['txtbatch_no']?>" id="bitch" data-parsley-group="step-1" data-parsley-required="true" class="form-control" readonly="true" /> 
            </div>
              </br>
              </br>
              <input type="hidden" name="id" value="11">
...
<!-- 
 ...
 -->                  </select>
              ...
              <div class="form-group row m-b-10">
                ...
                <div class="col-md-9">
                  <input type="number" class="form-control" placeholder="Insert Bin Number. . ." required="" readonly="" autofocus="" id="bin_no" />
                </div>
              </div>
              <div class="form-group row m-b-10">
                <input type="hidden" name="id" value="11">
...
$(document).ready(function(){
    ...
    <script type="text/javascript">
      var array = <?php echo json_encode($barcode_str); ?>;
        ...
        var batch_no      = "<?php echo $data['txtbatch_no'];?>";
        var formula_no    = "<?php echo $data['txtCode_Formula'];?>";
        var formula_desc  = "<?php echo $data['txtDesc_Formula'];?>";
        var formula_ver   = "<?php echo $data['txtVersion_Formula'];?>";
        var item_code     = "<?php echo $data['txtItemCode'];?>";
        var item_desc     = "<?php echo $data['txtItemDesc'];?>";
        var item_type     = "<?php echo $data['txtItemType'];?>";
        var qty_actual    = "<?php echo $data['intQty_Formula'];?>";
        var uom_scale     = "<?php echo $data['txtUom_Formula'];?>";
            ...
            swal({
              ...
              $.ajax({
                  ...
                  success : function(data){
                        window.location.href = "<?=site_url('c_transaction2/')?>"            
                  }
              });
            });
        }
    ...
    <script>
      function search(ele,value) {
        ...
        var okp = "<?php echo $data['txtbatch_no'];?>";
//        var blend = <?php echo $row2['total_item']; ?>;
    ...
    //                        $('#form_transaction').submit();
                          ...
                          if (nilai==blend) {
                            // input_tracebin("<?=$data['txtbatch_no']?>");
                            swal({
                              title: "SUCCESS",
                              text: "ITEM LENGKAP!",
                              icon: "success",
                              ...
                              timer: 2500,
                                ...
                                // input_tracebin("<?=$data['txtbatch_no']?>",<?=$counter?>,bin_no);
                                input_tracebin(kode,proses,bin_no,line_proses);
                                window.location.href = "<?=site_url('c_transaction2/transaction/').$data['txtbatch_no']?>"
    //                            $('#form_transaction').submit();
                            });
